...
checksum, deterministic artifact-manifest, and provenance workflow artifacts.
...
sign-time reachable upstream refs, and ZIP, DMG, checksum manifest, artifact manifest, and staged source
archive hashes. A fresh secret-free smoke job downloads the signed ZIP, validates the
provenance and artifact-manifest bindings, checks universal architectures, runs
the exact contained helper's early `--version` smoke, then launches the exact
contained app under isolated temporary `HOME`/`TMPDIR` state, completes a
bootstrap plus harmless `windows` request with that same helper, terminates only
the recorded child process, and removes the isolated state on every exit.
